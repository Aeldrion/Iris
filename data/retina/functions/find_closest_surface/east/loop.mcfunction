#> retina:find_closest_surface/east/loop
#
# Determines if the ray hits the current surface
#
# @within retina:find_closest_surface/east/main
...
execute if score ${x} retina > $x0 retina run function retina:find_closest_surface/east/get_distance
...
execute if score $surfaces retina matches 1.. run function retina:find_closest_surface/east/loop
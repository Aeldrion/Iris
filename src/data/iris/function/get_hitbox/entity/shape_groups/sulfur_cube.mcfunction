...
with open("resources/entity_types.json") as entities_file:
    ...
    if "minecraft:sulfur_cube" in entity_dimension_data:
        width = int(1e6 * entity_dimension_data["minecraft:sulfur_cube"]["width"])
        height = int(1e6 * entity_dimension_data["minecraft:sulfur_cube"]["height"])
        ...
        say Uh oh! Sulfur cubes don't exist yet! Someone should probably delete this file...
